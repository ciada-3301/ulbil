...
export default function CatalogueExplorer() {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="heritage-card p-6 rounded-2xl bg-white border border-[#E8E0D4] space-y-5">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-3.5 rounded-xl bg-[#F5F0E8] border-2 border-[#B8860B]/60 focus:border-[#D95D24] focus:outline-hidden text-sm transition-all text-[#2C2420]"
          />
        </div>
        ...
        <div className="space-y-3">
          <div>
            <span className="text-[11px] font-bold uppercase tracking-wider text-[#9A918A] block mb-1.5">
              Category Filter
            </span>
                ...
                <button
                  ...
                  className={`px-3 py-1 rounded-full text-sm font-semibold transition-all cursor-pointer ${
                    selectedCategory === cat
                      ...
                      : 'bg-[#F5F0E8] border border-[#E8E0D4] text-[#6B635D] hover:border-[#B8860B] hover:text-[#D95D24]'
                  }`}
                >
                  {cat}
                </button>
          ...
          <div className="flex items-center gap-2 pt-2 border-t border-[#E8E0D4]/40">
            <span className="text-[11px] font-bold uppercase tracking-wider text-[#9A918A]">Language:</span>
            <div className="flex items-center gap-1.5">
              {languages.map((lang) => (
                <button
                  key={lang}
                  onClick={() => setSelectedLanguage(lang)}
                  className={`px-2.5 py-0.5 rounded-md text-sm font-semibold transition-all cursor-pointer ${
                    selectedLanguage === lang
                      ? 'bg-[#8B6508] text-white'
                      : 'bg-[#F5F0E8] border border-[#E8E0D4] text-[#6B635D] hover:bg-[#FEF0EA]'
                  }`}
                >
                  {lang}
                </button>
      ...
      {/* Results Header */}
      <div className="flex items-center justify-between text-sm text-[#9A918A] px-1">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredBooks.map((book) => (
          <div
            key={book.id}
            className="heritage-card p-5 rounded-2xl bg-white border border-[#E8E0D4] flex flex-col justify-between group"
          >
            <div className="space-y-3">
            ...
            <div className="pt-4 mt-3 border-t border-[#E8E0D4]/60 flex items-center justify-between">
              <button
                onClick={() => setActiveBook(book)}
                className="text-sm font-bold text-[#D95D24] hover:text-[#D95D24] flex items-center gap-1.5 cursor-pointer"
              >
                <Eye className="w-3.5 h-3.5" />
                <span>View Details & Request</span>
              </button>
              <span className="text-[10px] font-mono text-[#8B6508]">{book.language}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      {filteredBooks.length === 0 && (
        <div className="p-12 text-center bg-white rounded-2xl border border-[#E8E0D4] space-y-3">
          <Book className="w-10 h-10 text-[#B8860B] mx-auto opacity-50" />
          <h3 className="font-serif font-bold text-base text-[#2C2420]">No volumes matched your search</h3>
          <p className="text-sm text-[#9A918A]">Try adjusting keywords or selecting a broader category filter.</p>
        </div>
      )}
      ...
      {activeBook && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl border-2 border-[#B8860B] max-w-lg w-full p-6 sm:p-8 shadow-2xl space-y-5 animate-in zoom-in-95 duration-200">
            ...
            <div className="flex items-start justify-between">
              <div className="space-y-1">
                ...
                <h3 className="font-serif font-bold text-xl text-[#2C2420] mt-1">
                  {activeBook.title}
                </h3>
              ...
              <button
                ...
                className="p-1.5 rounded-lg text-[#9A918A] hover:bg-[#FAF7F0] cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>

            <div className="p-4 rounded-xl bg-[#F5F0E8] border border-[#E8E0D4] text-sm space-y-1.5 text-[#4A4340]">
              ...
              <p><strong>Current Status:</strong> <span className="font-bold text-[#D95D24]">{activeBook.status}</span></p>
              {activeBook.isRare && (
            ...
            {requestSubmitted ? (
              ...
              <form onSubmit={handleHoldRequest} className="space-y-3">
                <div className="space-y-1">
                  <label className="block text-sm font-bold text-[#4A4340]">Your Membership Card Number / Name</label>
                  <input
                    type="text"
                    required
                ...
                <div className="flex items-center justify-end gap-3 pt-2">
                  <button
                    type="button"
                    onClick={() => setActiveBook(null)}
                    className="px-4 py-2 rounded-lg text-sm font-semibold text-[#6B635D] hover:bg-[#FAF7F0] cursor-pointer"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="px-5 py-2 rounded-xl text-sm font-bold text-white bg-[#D95D24] hover:opacity-95 shadow-xs cursor-pointer"
                  >
