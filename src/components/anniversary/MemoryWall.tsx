...
export default function MemoryWall() {
  ...
  return (
    <div id="memory-wall" className="space-y-12">
      ...
      <div className="heritage-card rounded-2xl bg-white border-2 border-[#DFB343]/60 p-6 sm:p-8 shadow-sm">
        <div className="flex items-center gap-3 mb-6">
          <div className="w-10 h-10 rounded-xl bg-[#FEF5ED] text-[#C2592B] flex items-center justify-center border border-[#DFB343]/40">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-xs font-bold text-[#3E3835] mb-1">City / Location</label>
              <input
                type="text"
                value={location}
            ...
            <div>
              <label className="block text-xs font-bold text-[#3E3835] mb-1">Your Association</label>
              <input
                type="text"
                value={association}
          ...
          <div>
            <label className="block text-xs font-bold text-[#3E3835] mb-1">Your Message or Memory *</label>
            <textarea
              required
              rows={3}
              value={message}
          ...
          <div className="flex justify-end">
            <button
              type="submit"
              className="px-6 py-2.5 rounded-xl text-xs font-bold text-white bg-gradient-to-r from-[#D95D24] to-[#C69214] hover:opacity-95 shadow-sm transition-all flex items-center gap-1.5 cursor-pointer"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {memories.map((mem) => (
          <div
            key={mem.id}
            className="heritage-card p-5 rounded-2xl bg-white border border-[#EADBCC] flex flex-col justify-between"
          >
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div className="w-7 h-7 rounded-full bg-[#FEF5ED] border border-[#DFB343]/60 flex items-center justify-center text-[#C2592B]">
                    <User className="w-3.5 h-3.5" />
                  </div>
                  <div>
                    <h4 className="font-serif font-bold text-xs text-[#221F1E]">{mem.name}</h4>
                    <p className="text-[10px] text-[#8C6D23]">{mem.association}</p>
                  </div>
                </div>
                <span className="text-[10px] text-[#A0720A]">{mem.date}</span>
              </div>

              <p className="text-xs text-[#5A504B] leading-relaxed italic bg-[#FAF7F0] p-3 rounded-lg border border-[#EADBCC]/40">
                &ldquo;{mem.message}&rdquo;
              </p>
            </div>

            <div className="pt-3 mt-2 border-t border-[#EADBCC]/40 flex items-center justify-between text-[10px] text-[#7A6E65]">
